...
React TS 项目额外规则见 .claude/rules/react/architecture.md 与 .claude/rules/react/naming.md。

- 严禁在任何代码/文档/对话中使用文本表情符号（✅❎❌🚫⭐等），用纯文本替代。
  详细见 .claude/rules/no-emoji.md。
...
## 4. 初始化避坑
首次安装强约束后，立即对全仓库运行一次格式化，避免存量文件的行尾空格/不可见字符
导致后续每次 commit 都产生纯空白 diff:
    find . -type f -name '*.py' -not -path './.venv/*' -not -path './ref/*' -exec sed -i'' 's/[ \t]\+$//g' {} \;
